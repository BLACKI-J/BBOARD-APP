...
const DirectoryFilters = ({ searchTerm, setSearchTerm, filterRole, setFilterRole, filterGroup, setFilterGroup, viewMode, setViewMode, groups, isMobile }) => {
    return (
        <div className="directory-filters df-wrap no-scrollbar" style={{
            ...
            {/* Search */}
            <div className="df-search" style={{ flex: '1 1 200px', minWidth: isMobile ? '140px' : '200px', position: 'relative' }}>
            ...
            {/* Filters + Toggle */}
            <div className="df-selects" style={{ display: 'flex', gap: '0.5rem', flexWrap: 'nowrap', alignItems: 'center', flexShrink: 0 }}>
                <select className="select-filter" value={filterRole} onChange={e => setFilterRole(e.target.value)}
                    style={{ background: 'white', border: '1.5px solid var(--glass-border)', borderRadius: '12px', padding: '0.5rem 0.75rem', fontWeight: '800', fontSize: '0.8rem', outline: 'none', cursor: 'pointer', minHeight: '40px' }}>
                ...
                {/* View toggle — desktop only (mobile = list forced) */}
                {!isMobile && (
