    ...
    return (
        <button type="button" onClick={() => onOpen(child.id)} className="glass-card"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', padding: '1rem 1rem 0.875rem' }}>
                <span style={{ display: 'inline-flex', borderRadius: '50%', flexShrink: 0, boxShadow: '0 0 0 2px rgba(255,255,255,0.85), 0 4px 12px oklch(40% 0.06 45 / 0.18)' }}>
            ...
            {sections.length > 0 ? (
                <div style={{ padding: '0 0.875rem 0.875rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    {sections.map(sec => (
                        <div key={sec.id} style={{ position: 'relative', background: `linear-gradient(180deg, rgba(255,255,255,0.5), rgba(255,255,255,0.05) 60%), color-mix(in oklch, ${sec.color} 11%, white)`, border: `1px solid color-mix(in oklch, ${sec.color} 24%, transparent)`, borderRadius: '16px', padding: '0.625rem 0.75rem', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.85)' }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem' }}>
                                {sec.rows.map((row, i) => (
                                    <span key={i} style={{ display: 'inline-flex', alignItems: 'center', fontSize: '0.72rem', fontWeight: '800', padding: '3px 10px', borderRadius: '100px', background: 'linear-gradient(180deg, rgba(255,255,255,0.95), rgba(255,255,255,0.72))', color: `color-mix(in oklch, ${sec.color} 78%, black)`, border: `1px solid color-mix(in oklch, ${sec.color} 24%, transparent)`, boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.9), 0 1px 2px rgba(60,32,8,0.05)' }}>
